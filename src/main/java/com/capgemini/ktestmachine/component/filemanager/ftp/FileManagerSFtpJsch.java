...
public class FileManagerSFtpJsch extends AFileManagerFtpApacheCommonsFwk implements
		FileManager {
	...
	private void contentToFile(String content, String filename,
		...
		try {
			...
			try {
				LOGGER.trace("Connecting to server: " + ftpHost);
				JSch jsch = new JSch();
				...
				LOGGER.trace("Changing the directory to: " + dir);
				...
				try {
					LOGGER.trace("Sending the file: " + filename);
					try {
