package com.capgemini.ktestmachine.component.cleanmanager.filesystem.ftp;
...
import com.capgemini.ktestmachine.utils.ftp.UtilsFtp;

public class CleanManagerFtp extends ACleanManagerFtpFwk implements CleanManager {
	...
	public void clean(List<Group> groups) throws ABaseException {
		LOGGER.trace("BEGIN");
		try {
			testConfigured();

			if (groups == null || groups.isEmpty()) {
				LOGGER.warn("No group to remove");
			}
	...
	private Set<FTPFile> workDirRemoveFiles(FTPClient client, String pathDir,
			List<Pattern> patterns) throws ABaseException {
		try {
